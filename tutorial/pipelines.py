...
class MySQLStoreCnblogsPipeline(object):
    ...
    def _do_upinsert(self, conn, item, spider):
        ...
        conn.execute("""
                select 1 from home_cnblogsinfo where linkmd5id = %s
        """, (linkmd5id, ))
        ret = conn.fetchone()

        if ret:
            conn.execute("""
                update home_cnblogsinfo set title = %s, description = %s, link = %s, listUrl = %s, updated = %s where linkmd5id = %s
            ...
            conn.execute("""
                insert into home_cnblogsinfo(linkmd5id, title, description, link, listUrl, updated)
                values(%s, %s, %s, %s, %s, %s)
